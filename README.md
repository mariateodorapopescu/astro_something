...
**Istoric de calcule:** cand esti logat, fiecare calcul (individual, partnership, human design, ascendant) se leaga de contul tau (coloana `user_id` din fiecare tabel) si apare in lista unificata de pe `/account` (`GET /api/my-calculations`, sortata dupa data). Poti sterge orice element (`DELETE /api/my-calculations/{kind}/{id}` — sterge doar daca e al tau). Calculele facute fara cont raman anonime (`user_id = NULL`).
...
| `GET` | `/api/my-calculations` | istoricul (toate tipurile) al userului (**cere token**) |
| `DELETE` | `/api/my-calculations/{kind}/{id}` | sterge un calcul din istoric (**cere token**) |
